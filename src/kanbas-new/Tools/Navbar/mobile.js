...
function MobileNavbar() {
	...
	return (
		<div id="navbar" className="wd-small-navbar list-group">
			{link.map((link, index) => (
				<li className="wd-small-navbar-item" key={index}>
					<Link key={index} to={`/Kanbas/${link}`} className={`wd-small-navbar-link ${pathname.includes(link) && "active"}`}>
						<div className="wd-small-navbar-icon-container" aria-hidden="true">
							{fetchIcon(link)}
						</div>
						<div className="wd-small-navbar-text">{link}</div>
					</Link>
				</li>
			))}
		</div>
